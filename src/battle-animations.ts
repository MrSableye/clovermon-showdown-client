...
const clovercapTracks: Track[] = [
	...
	{ file: 'audio/battle-gymleader.mp3', start: 2748, end: 84685 },
];
...
export class BattleScene implements BattleSceneStub {
	...
	updateGen() {
		let forceGen3 = ['cloveronly', 'clovercaponly', 'cloverblobboscaponly', 'sburbmonsonly', 'sweetonly', 'clovertestonly', 'copeonly', 'copecapcabdraftonly', 'wackcaponly'];
		...
		for (let siden = 0; siden < 2 || (this.battle.gameType === 'multi' && siden < 4); siden++) {
			...
			for (let i = 0; i < side.pokemon.length; i++) {
				...
				let spriteData = Dex.getSpriteData(pokemon, !!spriteIndex, {
					gen: this.gen,
					noScale: true,
					mod: this.mod || this.battle.modName || '',
				});
				let y = 0;
				let x = 0;
				if (spriteIndex) {
	...
	upkeepWeather() {
		const isIntense = ['desolateland', 'primordialsea', 'deltastream', 'hyperboreanarctic'].includes(this.curWeather);
	...
	updateWeather(instant?: boolean) {
		...
		if (weather === 'desolateland' || weather === 'primordialsea' || weather === 'deltastream' || weather === 'hyperboreanarctic') {
			isIntense = true;
		}
...
export class PokemonSprite extends Sprite {
	...
	animSub(instant?: boolean, noAnim?: boolean) {
		...
		const subsp = Dex.getSpriteData('substitute', this.isFrontSprite, {
			...
			mod: this.scene.mod || this.scene.battle.modName || '',
		});
		this.subsp = subsp;
	...
	updateStatbar(pokemon: Pokemon, updatePrevhp?: boolean, updateHp?: boolean) {
		...
		} else if (pokemon.status === 'radish') {
			status += '<span class="radish">RADISH</span> ';
		}
		if (pokemon.terastallized) {
